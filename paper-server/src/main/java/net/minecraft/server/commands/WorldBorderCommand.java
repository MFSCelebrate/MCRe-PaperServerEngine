...
public class WorldBorderCommand {
    ...
    private static final SimpleCommandExceptionType ERROR_TOO_BIG = new SimpleCommandExceptionType(
        Component.translatable("commands.worldborder.set.failed.big", 6.7104000E7F)
    );
    private static final SimpleCommandExceptionType ERROR_TOO_FAR_OUT = new SimpleCommandExceptionType(
        Component.translatable("commands.worldborder.set.failed.far", 3.3552000e7)
    );
            ...
            Commands.literal("worldborder")
                ...
                .then(
                    Commands.literal("add")
                        .then(
                            Commands.argument("distance", DoubleArgumentType.doubleArg(-6.7104000E7F, 6.7104000E7F))
                                .executes(
                                    context -> setSize(
                                        context.getSource(),
    ...
    private static int setCenter(CommandSourceStack source, Vec2 pos) throws CommandSyntaxException {
        ...
        } else if (!(Math.abs(pos.x) > 3.3552000e7) && !(Math.abs(pos.y) > 3.3552000e7)) {
